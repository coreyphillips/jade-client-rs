//! Public data types.
//!
//! These are plain Rust types with no binding framework attached. An adapter
//! that needs to expose them across a foreign function boundary can attach
//! scaffolding externally, for example with UniFFI's `#[uniffi::remote]`,
//! rather than maintaining a mirrored set of structs.
//!
//! Wire shapes stay private: Jade's `get_version_info` reply uses
//! SCREAMING_SNAKE keys and a string state, so deriving `Deserialize` straight
//! onto the public type would silently yield nothing but `None`.
...
pub const MIN_JADE_FIRMWARE: &str = "0.1.48";

/// Taproot address support landed in 1.0.34.
pub const MIN_JADE_FIRMWARE_TAPROOT: &str = "1.0.34";
...
impl JadeAddressVariant {
    /// The variant conventionally derived under a given BIP44 purpose.
    pub fn from_purpose(purpose: u32) -> Option<Self> {
        match purpose {
            44 => Some(JadeAddressVariant::Pkh),
            49 => Some(JadeAddressVariant::ShWpkh),
            84 => Some(JadeAddressVariant::Wpkh),
            86 => Some(JadeAddressVariant::Tr),
            _ => None,
        }
    }
}
...
pub struct JadeDeviceInfo {
    /// Transport specific address: a serial device path or a BLE identifier.
    pub path: String,
