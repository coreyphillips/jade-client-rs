...
use crate::transport::JadeConnection;
...
#[async_trait]
pub trait PinServerHttp: Send + Sync {
...
/// The bundled implementation, over `reqwest`.
///
/// Applies the URL and address hardening documented on [`validate_url`] and
/// [`resolve_and_validate`]. A consumer that needs different networking, for
/// example a proxy or Tor, can implement [`PinServerHttp`] instead.
#[cfg(feature = "reqwest-pinserver")]
#[derive(Debug, Default, Clone, Copy)]
pub struct ReqwestPinServer;
...
///
/// The URL list arrives from the device, so a tampered or second hand unit can
/// point the host anywhere. Requests are limited to https on port 443, with no
/// credentials, no onion host and no redirects.
#[cfg(feature = "reqwest-pinserver")]
pub fn validate_url(url: &str) -> Result<url::Url, JadeError> {
...
///
/// The resolved address is pinned onto the client so a second lookup cannot
/// return a different one, which closes the DNS rebinding window.
#[cfg(feature = "reqwest-pinserver")]
pub async fn resolve_and_validate(url: &url::Url) -> Result<std::net::SocketAddr, JadeError> {
...
pub fn cbor_to_json(value: &ciborium::Value) -> Result<serde_json::Value, JadeError> {
...
pub fn json_to_cbor(value: &serde_json::Value) -> Result<ciborium::Value, JadeError> {
    Ok(match value {
