...
/// A typed cause a transport can attach to a failure.
///
/// Kept as a real enum rather than an error string so an adapter layer, for
/// example a foreign function binding, can map a host side failure onto a
/// `JadeError` without encoding the reason into text and parsing it back out.
...
impl From<JadeTransportErrorCode> for JadeError {
    fn from(code: JadeTransportErrorCode) -> Self {
        match code {
            JadeTransportErrorCode::DeviceBusy => JadeError::DeviceBusy,
            JadeTransportErrorCode::NotConnected => JadeError::NotConnected,
            JadeTransportErrorCode::Disconnected => JadeError::DeviceDisconnected,
            JadeTransportErrorCode::Timeout => JadeError::Timeout,
            JadeTransportErrorCode::PermissionDenied => JadeError::TransportError {
...
impl JadeError {
    ...
    pub fn protocol(details: impl std::fmt::Display) -> Self {
    ...
    pub fn transport(details: impl std::fmt::Display) -> Self {
    ...
    pub fn from_rpc(code: i64, message: String, min_firmware: &str) -> Self {
        match code {
